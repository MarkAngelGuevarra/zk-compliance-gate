/**
 * index.js — ZK Compliance Gate Main Page
 *
 ...
 *   - User enters private age → stays in browser memory only
 *   - ZK circuit proves age >= threshold
 *   - ONLY true/false result written to Midnight Preprod ledger
 *   - Private age is NEVER transmitted, logged, or stored
 */
        ...
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div style={{
              ...
              <p style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--success)', marginBottom: '0.5rem' }}>
                🌐 PUBLIC (On-Chain)
              </p>
              {[
                'Caller\'s Midnight address',
                'Result: eligible (true/false)',
                'Threshold used (e.g., 18)',
                'Block timestamp',
                'Total verification count',
              ].map(item => (
                <div key={item} style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', padding: '0.15rem 0' }}>
                  ✓ {item}
                </div>
              ))}
            </div>
            ...
            <div style={{
              ...
              <p style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--error)', marginBottom: '0.5rem' }}>
                🔒 PRIVATE (ZK Circuit Only)
              </p>
              {[
                'Actual age value',
                'Any number > threshold',
                'Any number < threshold',
